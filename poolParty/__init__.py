...
class Pool:
    def __init__(self, max_workers=os.cpu_count() + 4, name=None, queue=None, wait_queue=True, result_queue=None,
                 workers_sleep=0.5, callback=None, exception_handler=exception_handler):
        self.name = name
    ...
    def start(self):
        ...
        for i in range(self.max_workers):
            self.threads.append(Worker(f'Worker_{i}_{self.name}', self.queue, self.result_queue,
                                       wait_queue=self.wait_queue, sleep=self.workers_sleep, callback=self.callback))
    ...
    def update(self, n, block=False):
        ...
        if need > 0:
            # create more threads
            for _ in range(need):
                t = Worker(f'Worker_{(self.count())} ({self.name})', self.queue, self.result_queue,
                           wait_queue=self.wait_queue, callback=self.callback)
                t.start()
